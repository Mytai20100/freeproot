...
int main(int argc, char *const argv[])
{
	...
	/* --- Load proot.yml BEFORE parsing CLI args --- */
	if (load_yml_config(&yml_cfg) == 0) {
		yml_loaded = true;
		/* We need the CLI object to call handlers; get it early */
		const Cli *cli = get_proot_cli(tracee->ctx);
		apply_yml_config(tracee, cli, &yml_cfg);
	}
	...
	status = parse_config(tracee, effective_argc, effective_argv);
	if (status < 0)
		goto error;

	/* Start the first tracee.  */
	status = launch_process(tracee, &effective_argv[status]);
	if (status < 0) {
